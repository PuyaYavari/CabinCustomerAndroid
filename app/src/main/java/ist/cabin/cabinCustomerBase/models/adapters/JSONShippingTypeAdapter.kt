...
import ist.cabin.cabinCustomerBase.models.backend.JSONShippingType

class JSONShippingTypeAdapter (moshi: Moshi) : JsonAdapter<JSONShippingType>() {
    ...
    @FromJson
    override fun fromJson(reader: JsonReader): JSONShippingType? {
        var id: Int? = null
        ...
        while (reader.hasNext()) {
            try {
                ...
                Log.e("JSONShippingTypeAdapter", exception.message.toString())
            }
        }
        reader.endObject()
        return try {
            val result = JSONShippingType(
                id = id!!,
                name = name!!
            )
            result
    ...
    @ToJson
    override fun toJson(writer: JsonWriter, value: JSONShippingType?) {
        if (value == null) {
